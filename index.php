...
// Change app welcome message depending on context
if (!empty ($_REQUEST['signed_request'])) {
  // canvas app
  $app_welcome_name = 'Facebook canvas app';
} else {
  // external site app
  $app_welcome_name = 'Facebook app';
}
...
if ($token) {
  ...
  $likes = array_values(
    idx(FBUtils::fetchFromFBGraph('me/likes?access_token='.$token.
      '&limit=4'), 'data')
  );

  // This fetches 4 of your friends.
  $friends = array_values(
    idx(FBUtils::fetchFromFBGraph('me/friends?access_token='.$token.
      '&limit=4'), 'data')
  );

  // And this returns 16 of your photos.
  $photos = array_values(
    idx($raw = FBUtils::fetchFromFBGraph('me/photos?access_token='.$token.
      '&limit=16'), 'data')
  );
  ...
  $app_using_friends = FBUtils::fql(
    'SELECT uid, name, is_app_user, pic_square FROM user WHERE uid in'.
  ...
  $send_url = 'https://www.facebook.com/dialog/send?redirect_uri='.
    $redirect_url.'&display=popup&app_id='.$app_id.'&link='.$encoded_home;
  $post_to_wall_url = 'https://www.facebook.com/dialog/feed?redirect_uri='.
    $redirect_url.'&display=popup&app_id='.$app_id;
} else {
...
<html lang="en">
  ...
  <body>
    <header class="clearfix">
      ...
      <div>
        <h1>Welcome, <strong><?php echo html_entity_decode(idx($basic, 'name')); ?></strong></h1>
        <p class="tagline">
          This is your app
    ...
    <section id="get-started">
      <p>Welcome to your <?php echo $app_welcome_name; ?>, running on <span>heroku</span>!</p>
        ...
        <ul class="friends">
          <?php
            foreach ($friends as $friend) {
              ...
              $name = html_entity_decode(idx($friend, 'name'));
        ...
        <ul class="photos">
          <?php
            foreach ($photos as $key => $photo) {
              // Extract the pieces of info we need from the requests above
              $src = html_entity_decode(idx($photo, 'source'));
              $name = html_entity_decode(idx($photo, 'name'));
        ...
        <ul class="things">
          <?php
            foreach ($likes as $like) {
              ...
              $item = html_entity_decode(idx($like, 'name'));
        ...
        <ul class="friends">
          <?php
            foreach ($app_using_friends as $auf) {
              ...
              $pic = html_entity_decode(idx($auf, 'pic_square'));
              $name = html_entity_decode(idx($auf, 'name'));
              echo('
                <li>
